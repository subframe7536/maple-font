...
| Runtime decisions      | `config/runtime.py`                                      | Resolve output paths and runtime flags.                                                                                  |
| CJK configuration      | `cjk/config.py`, `cjk/cli.py`, `cjk/masters.py`, `cjk/paths.py` | Parse CJK JSON and CLI inputs, validate source masters, and derive locale-specific paths.                     |
| CJK base resolution    | `cjk/resolver.py`, `cjk/assets.py`, `cjk/cache.py` | Resolve reusable bases with validated archive installation and static/variable/source fallback.                              |
...
| Shared font operations | `font_ops/`                                             | Keep naming, hinting, metrics, glyph transforms, OpenType edits, merging, subsetting, and FontTools boundaries reusable. |
